...
import 'schema/masterclass_zone_record.dart';
...
export 'schema/masterclass_zone_record.dart';
...
/// Functions to query MasterclassZoneRecords (as a Stream and as a Future).
Future<int> queryMasterclassZoneRecordCount({
  Query Function(Query)? queryBuilder,
  int limit = -1,
}) =>
    queryCollectionCount(
      MasterclassZoneRecord.collection,
      queryBuilder: queryBuilder,
      limit: limit,
    );

Stream<List<MasterclassZoneRecord>> queryMasterclassZoneRecord({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
    queryCollection(
      MasterclassZoneRecord.collection,
      MasterclassZoneRecord.fromSnapshot,
      queryBuilder: queryBuilder,
      limit: limit,
      singleRecord: singleRecord,
    );

Future<List<MasterclassZoneRecord>> queryMasterclassZoneRecordOnce({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
  ...
  return query.snapshots().handleError((err) {
    print('Error querying $collection: $err');
      ...
      .map(
        (d) => safeGet(
          () => recordBuilder(d),
          (e) => print('Error serializing doc1 ${d.reference.path}:\n$e'),
        ),
      )
      .where((d) => d != null)
      .map((d) => d!)
...
Future<List<T>> queryCollectionOnce<T>(
  ...
  return query.get().then((s) => s.docs
      .map(
        (d) => safeGet(
          () => recordBuilder(d),
          (e) => print('Error serializing doc2 ${collection} ${d.reference.path}:\n$e'),
        ),
      )
      .where((d) => d != null)
      .map((d) => d!)
...
Future<FFFirestorePage<T>> queryCollectionPage<T>(
  ...
  final getDocs = (QuerySnapshot s) => s.docs
      .map(
        (d) => safeGet(
          () => recordBuilder(d),
          (e) => print('Error serializing doc3 ${d.reference.path}:\n$e'),
        ),
      )
      .where((d) => d != null)
      .map((d) => d!)
