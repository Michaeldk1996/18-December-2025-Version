...
import 'gold_advance_subscription_model.dart';
export 'gold_advance_subscription_model.dart';

class GoldAdvanceSubscriptionWidget extends StatefulWidget {
  final HomeModel homeModel;
  GoldAdvanceSubscriptionWidget({super.key, required this.homeModel});

  @override
  State<GoldAdvanceSubscriptionWidget> createState() =>
      _GoldAdvanceSubscriptionWidgetState();
}

class _GoldAdvanceSubscriptionWidgetState
    extends State<GoldAdvanceSubscriptionWidget> {
  late GoldAdvanceSubscriptionModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => GoldAdvanceSubscriptionModel());
                                ...
                                Padding(
                                  ...
                                  child: FFButtonWidget(
                                    onPressed: () async {
                                      ...
                                      widget.homeModel.settingsResponse =
                                          await querySettingsRecordOnce(
                                        singleRecord: true,
                                      ).then((s) => s.firstOrNull);
                                      _shouldSetState = true;
                                      if (widget.homeModel.settingsResponse!.approve) {
                                        logFirebaseEvent('Button_backend_call');
                                        widget.homeModel.apiResultd8s =
                                        ...
                                        _shouldSetState = true;
                                        if ((widget.homeModel.apiResultd8s?.succeeded ?? true)) {
                                          logFirebaseEvent('Button_launch_u_r_l');
                                          await launchURL(getJsonField(
                                            (widget.homeModel.apiResultd8s?.jsonBody ?? ''),
                                            r'''$.redirectUrl''',
                                          ).toString());
                                        }
                                      } else {
                                        ...
                                        widget.homeModel.didPurchaseUpgrade = await revenue_cat
                                            .purchasePackage(revenue_cat.offerings!.current!
                      .getPackage(
                          'bspconsult_upgrade_gold_masterclass')!
                      .identifier);
                                        _shouldSetState = true;
                                        if (widget.homeModel.didPurchaseUpgrade!) {
                                          logFirebaseEvent('Button_backend_call');
